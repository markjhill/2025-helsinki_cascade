...
if(exists("my_term")) {
  focused_dfm <- subset_by_date(rsc_dfm, 
                                decade = peak_decade)  # or choose your own
  
  # How does your term's usage vary by document type in this period?
  for(dtype in unique(docvars(focused_dfm)$type)) {
    type_subset <- subset_by_type(focused_dfm, dtype)
...
# If you started with TIME, add a topic constraint:
if(exists("my_period_dfm")) {
  ...
  my_topic <- "______"  # <-- CHOOSE ONE
  
  focused_dfm <- subset_by_topic(my_period_dfm, topics = my_topic)
  print(paste("Documents about", my_topic, "in your period:", ndoc(focused_dfm)))
  
  # What are the key terms for this topic in your period?
  topfeatures(focused_dfm, 20)
}
...
```{r 3.2}
# Create your double-filtered subset from above
# Name it focused_dfm if you haven't already
...
if(exists("my_term")) {
  ...
  decades_to_compare <- c(peak_decade - 20, peak_decade, peak_decade + 20)

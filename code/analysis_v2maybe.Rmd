...
```{r load-metadata}
# Load metadata
df <- read.csv("C:\\Users\\markj\\onedrive_cloud\\coding\\GitHub\\2025-helsinki_cascade\\data\\rsc\\Royal_Society_Corpus_open_v6.0_meta.tsv", 
               sep = "\t", 
...
```{r check-ids}
# The file ID is simply the id column from metadata
# Files are named: Royal_Society_Corpus_open_v6.0_text_{id}.txt
...
text_dir <- "C:\\Users\\markj\\OneDrive - King's College London\\coding\\GitHub\\2025-helsinki_cascade\\data\\rsc\\Royal_Society_Corpus_open_v6.0_texts_txt\\"
...
network_data <- read.csv("C:\\Users\\markj\\onedrive_cloud\\coding\\GitHub\\2025-helsinki_cascade\\data\\rsc\\multi_temporal_communities.csv")
...
# Display sample of network data
head(network_data) %>% DT::datatable(options = list(scrollX = TRUE))
```
...
# Process all author strings
df_complete$authors_list <- map(df_complete$author, process_authors)

# Create additional useful author variables
df_complete$author_count <- map_int(df_complete$authors_list, length)
df_complete$first_author <- map_chr(df_complete$authors_list, ~ ifelse(length(.x) > 0, .x[1], NA_character_))
...
# Show examples of author processing
author_examples <- df_complete %>%
  select(id, author, author_count, first_author, is_multi_author) %>%
  head(10)

print("Author processing examples:")
DT::datatable(author_examples, options = list(scrollX = TRUE))
```

## Community Analysis Functions (Operating on Separate Network Data)
...
# Examples of using network data functions
cat("=== NETWORK DATA EXAMPLES ===\n")
...
```{r debug-data}
# Let's check what's in our corpus docvars
cat("=== DEBUGGING CORPUS DATA ===\n")
...
cat("=== DEBUGGING SUBSET_CORPUS FUNCTION ===\n")

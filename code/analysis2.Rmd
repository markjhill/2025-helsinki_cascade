...
```{r baseline-analysis}
cat("=== ANALYSIS 1: ALL TEXT (NO METADATA SUBSETTING) ===\n")
cat("Question: What are the dominant concepts in Royal Society publications?\n\n")

# Create basic corpus for analysis (full dataset)
full_corpus <- rsc_corpus

# Basic corpus overview
cat("Corpus Overview:\n")
cat("Total documents:", ndoc(full_corpus), "\n")
cat("Total tokens:", sum(ntoken(full_corpus)), "\n")
cat("Date range:", min(docvars(full_corpus, "year"), na.rm = TRUE), "to", 
    max(docvars(full_corpus, "year"), na.rm = TRUE), "\n\n")

...
- visualisationLink: Link to any digital visualisations of the document
...
The RSC includes topic classifications that reveal the disciplinary structure of early modern science. These categories may reflect how scientific knowledge was organised in different historical periods.
...
Finally, once a corpus is subset, you can examine the documents that remain.

```{r corpus_docs}
corpus_comm1_authors

full_text_test <- as.character(corpus_comm1_authors)[3]   # get the third document
# cat(full_text_test) # print the whole thing - uncomment to see
```
...
print(test_dfm)
```
It is the frequency and relationship of these features (tokens, words) that is central to NLP. We will, therefore, be analysing DFMs which are quantitative representation of our documents - figuratively "bags of words." *N.B.* This also means that the human-readable text is no longer there. If you want to read a document you will need to go back to the corpus object.

For example, it becomes easy (and computationally less difficult) to identify the most frequent words (or other features):

```{r test_mfw}
topfeatures(test_dfm)
```

It is, therefore, a DFM of the RSC that we will analyse going forward. However, to do this we first need to construct it.

*N.B.:* this can take a few minutes, depending on computational resources. An already constructed DFM can be loaded by commenting out code:
...
end_time <- Sys.time()
cat("Total time taken to tokenise and create DFM:")
print(end_time-start_time)

#saveRDS(rsc_dfm, file = "../data/rsc_dfm.RDS")
```

And now we can begin to analyse the text.

```{r rsc_dfm_and_mff}
topfeatures(rsc_dfm, n = 50)
```

However, we can quickly see that many of these "words" are not meaningful. We therefore may also want to create our own constum stopword list.
...
topfeatures(rsc_dfm, n = 50)
```
You can recreate (or reload) the DFM at any point if you think you've removed too many.
...
# Most frequent terms
cat("Top 50 Most Frequent Terms (All Documents):\n")
topfeatures(rsc_dfm, 50) %>% print()

# Most frequent n-grams
rsc_bigrams <- rsc_tokens %>%
  tokens_remove(c(custom_stopwords)) %>%
  tokens_ngrams(n = 2) %>% 
  dfm()
cat("Top 50 Most Frequent Bigrams:\n")
topfeatures(rsc_bigrams, 50) %>% print()
...
**Research Questions**: How did scientific concepts evolve over time? When did new terminology emerge?
...
# Create by century
dfm_1700s <- subset_corpus(rsc_dfm, century = 1700)
dfm_1800s <- subset_corpus(rsc_dfm, century = 1800)

# # Create or by start and end year
# dfm_1700s <- subset_corpus(rsc_dfm, start_year = 1700, end_year = 1799)
# dfm_1800s <- subset_corpus(rsc_dfm, start_year = 1800, end_year = 1899)

# Overview of temporal subsets
cat("\n1700s Corpus:\n") 
cat("Documents:", ndoc(dfm_1700s), "\n")
cat("Tokens:", sum(ntoken(dfm_1700s)), "\n\n")

cat("\n1800s Corpus:\n")
cat("Documents:", ndoc(dfm_1800s), "\n")
cat("Tokens:", sum(ntoken(dfm_1800s)), "\n")
...
This is quite insightful at first glance, but top features alone only tell us so much. What about less frequent, but completely new words? For that we can look at relative frequency (keyness).

```{r relative_freq}

# Compare 1700s vs 1800s using keyness analysis
# This is statistically rigorous and much simpler than manual frequency ratios

# Combine the DFMs and add period labels
combined_dfm <- rbind(dfm_1700s, dfm_1800s)
docvars(combined_dfm, "period") <- c(rep("1700s", ndoc(dfm_1700s)),
                                     rep("1800s", ndoc(dfm_1800s)))

# Calculate keyness (target = TRUE means 1800s vs 1700s)
keyness_results <- textstat_keyness(combined_dfm, 
                                    target = docvars(combined_dfm, "period") == "1800s")

# Terms that became much more prominent in 1800s
cat("\nTerms most distinctive of 1800s (became much more frequent):\n")
keyness_results %>% 
  filter(chi2 > 0) %>% 
  arrange(desc(chi2)) %>% 
  head(10) %>% 
...
# Terms that were more characteristic of 1700s (declined in 1800s)
cat("\nTerms most distinctive of 1700s (declined in 1800s):\n")
keyness_results %>% 
  filter(chi2 < 0) %>% 
  arrange(chi2) %>% 
  head(10) %>% 
...
The most intuitive measurements here are n_target and n_reference. This is how many more times the word (feature) appears in one subset than the other. In this case, temperature occurs almost 30x more frequently (38399/1291). That is a strong effect size.

Additionally, chi2 is a statistical measurement the greater it is, the stronger the association between the things you're looking at. In this case temperature with a chi2 of > 5000 is an extremely strong association. In other words, it would be unlikely for temperature to have been in one subset that many times more often than the other by chance. The p value reports whether the chi2 is statistically significant.

When exploring the features:

Look for:

- Emerging terminology: E.g., new scientific concepts that appeared in the 1800s
- Declining usage: How certain 18th-century terms that became less common
- Persistent concepts: Terms that remained important across both periods

This analysis can reveal scientific developments like the emergence of electromagnetic theory, advances in chemistry, or new measurement techniques.

Quite useful! So here's another example:

```{r freq_function}
# # Create or by start and end year
dfm_1600s <- subset_corpus(rsc_dfm, start_year = 1600, end_year = 1700)
dfm_1700s <- subset_corpus(rsc_dfm, start_year = 1700, end_year = 1799)

# Combine the DFMs and add period labels
combined_dfm <- rbind(dfm_1600s, dfm_1700s)
docvars(combined_dfm, "subset_") <- c(rep("1600s", ndoc(dfm_1600s)),
                                     rep("1700s", ndoc(dfm_1700s)))

# Calculate keyness (target = TRUE means 1600s vs 1600s)
keyness_results <- textstat_keyness(combined_dfm, 
                                    target = docvars(combined_dfm, "subset_") == "1700s")

# Terms that became much more prominent in 1700s
cat("\nTerms most distinctive of 1700s (became much more frequent):\n")
keyness_results %>% 
  filter(chi2 > 0) %>% 
  arrange(desc(chi2)) %>% 
  head(10) %>% 
...
# Terms that were more characteristic of 1600s (declined in 1700s)
cat("\nTerms most distinctive of 1600s (declined in 1700s):\n")
keyness_results %>% 
  filter(chi2 < 0) %>% 
  arrange(chi2) %>% 
  head(10) %>% 
...
## 3. Topical Metadata: Disciplinary Language

**Research Questions:**

- How do different scientific fields use language distinctively?
- What terminology is field-specific vs. shared across disciplines?
- How do interdisciplinary boundaries manifest in language use?
- Which fields are more specialised vs. more accessible in their terminology?

Disciplinary Context: The Royal Society period (1665-1920) predates modern academic departments, so these topic classifications reflect how language may reflect undefined disciplinary boundaries. This could provide insight into the historical organisation and/or emergence of scientific knowledge before the professionalisation and specialisation that characterises modern science.

```{r topic-analysis}

# Create topic-based subsets (using first two most common topics)
dfm_topic1 <- subset_corpus(rsc_dfm, topics = "Biology 2")
dfm_topic2 <- subset_corpus(rsc_dfm, topics = "Astronomy")

# Overview of topic subsets
cat("Subset 1:\n")
cat("Documents:", ndoc(dfm_topic1), "\n")
cat("Tokens:", sum(ntoken(dfm_topic1)), "\n")

cat("Subset 2:\n")
cat("Documents:", ndoc(dfm_topic2), "\n")
cat("Tokens:", sum(ntoken(dfm_topic2)), "\n\n")

# Compare vocabularies
cat("Top 15 Terms in subset 1:\n")
topfeatures(dfm_topic1, 15) %>% print()

cat("Top 15 Terms in subset 2:\n")
topfeatures(dfm_topic2, 15) %>% print()
# 
# # Calculate distinctive terms
# combined_topic_dfm <- rbind(dfm_topic1, dfm_topic2)
# docvars(combined_topic_dfm, "subset") <- c(rep("subset1", ndoc(dfm_topic1)),
#                                           rep("subset2", ndoc(dfm_topic2)))
# 
# topic_keyness <- textstat_keyness(combined_topic_dfm, 
#                                   target = docvars(combined_topic_dfm, "subset") == "subset1")
# 
...
combined_dfm <- rbind(dfm_topic1, dfm_topic2)
docvars(combined_dfm, "subset") <- c(rep("subset1", ndoc(dfm_topic1)),
                                        rep("subset2", ndoc(dfm_topic2)))

# Calculate keyness
keyness_results <- textstat_keyness(combined_dfm, 
                                  target = docvars(combined_dfm, "subset") == "subset1")

cat("\nTerms most distinctive of subset1:\n")
keyness_results %>% 
  filter(chi2 > 0) %>% 
  arrange(desc(chi2)) %>% 
  head(10) %>% 
...
cat("\nTerms most distinctive of subset2:\n")
keyness_results %>% 
  filter(chi2 < 0) %>% 
  arrange(chi2) %>% 
  head(10) %>% 
...
### Interpreting Topical Results

Look for:

- Technical vocabularies: Specialised terminology unique to each field
- Methodological language: Different approaches to investigation and measurement
- Theoretical frameworks: How fields conceptualise and explain phenomena
- Cross-field terminology: Concepts that bridge disciplines vs. field-specific jargon

This analysis may reveal how scientific disciplines maintain their distinctiveness through language while also sharing common ground where fields intersect.


## 4. Combining Metadata

When multiple perspectives are brought together one can ask more complex questions. For example:

**Research Questions**: 

- How do multiple metadata dimensions interact to shape scientific discourse?
- What happens when we layer temporal, topical, and social factors together?
- How can we trace the evolution of specific scientific communities or disciplines?
- What complex patterns emerge only when combining different types of context?

**Methodological Strength(s):** This represents the full potential of metadata-driven analysis—the ability to ask complex, multi-faceted questions that would be impossible with any single dimension of context.

Example Applications:

- Tracking how specific scientific communities evolved their language over time
- Comparing how different fields adopted new terminology during the same period
- Analysing how collaboration patterns vary across disciplines and historical periods
- Investigating how major scientific discoveries propagated through different networks
...
dfm_phys_1800s <- subset_corpus(rsc_dfm, 
                              topics = physics_topics,
                              start_year = 1800, end_year = 1899)

dfm_phys_1900s <- subset_corpus(rsc_dfm,
                              topics = physics_topics, 
                              start_year = 1900, end_year = 1920)

cat(physics_topics, "in 1800s:\n")
cat("Documents:", ndoc(dfm_phys_1800s), "\n")

cat("\n", physics_topics, "in 1900s:\n")
cat("Documents:", ndoc(dfm_phys_1900s), "\n\n")

cat("Top terms in", physics_topics, "1800s:\n")
topfeatures(dfm_phys_1800s, 10) %>% print()

cat("\nTop terms in", physics_topics, "1900s:\n")
topfeatures(dfm_phys_1900s, 10) %>% print()

# Find evolving terminology
combined_phys_dfm <- rbind(dfm_phys_1800s, dfm_phys_1900s)
docvars(combined_phys_dfm, "subset") <- c(rep("subset1", ndoc(dfm_phys_1800s)),
                                          rep("subset2", ndoc(dfm_phys_1900s)))

phys_keyness <- textstat_keyness(combined_phys_dfm,
                                 target = docvars(combined_phys_dfm, "subset") == "subset2")

cat("\nTerms that became more prominent in", physics_topics, "by 1900s:\n")
phys_keyness %>%
  filter(chi2 > 0) %>%
  arrange(desc(chi2)) %>%
  head(8) %>%
...
- Community Evolution: Track how specific intellectual communities changed their research focus over time
- Cross-Field Diffusion: Study how terminology spreads from one discipline to another
- Collaboration and Innovation: Examine whether collaborative works introduce new concepts faster than individual works

## 5. Text as variable

In the previous sections, we used metadata to subset text and compare different groups. Now we flip the approach: we use text patterns to investigate the metadata itself. This represents a fundamental shift in analytical perspective—instead of asking "How do different groups use language?", we ask "What can language patterns tell us about historical, social, or institutional phenomena?"

Research Questions Enabled:

When did specific scientific concepts or terms first emerge and spread?
Which authors were most central to developing particular fields or ideas?
How can we trace the diffusion of ideas through scientific communities?
What can linguistic patterns reveal about the social organisation of science?

Methodological Insight: Text becomes a lens for understanding broader patterns in scientific history, authorship, and knowledge production. We treat word usage, terminology evolution, and linguistic patterns as evidence for historical processes.

### 5.1 Temporal Emergence: When Did Scientific Concepts Appear?

Rather than comparing predefined time periods, we can trace when specific terms entered scientific discourse and how their usage evolved.

```{r text_as_var}
# Track emergence of key scientific terms
electricity_usage <- track_term_over_time(rsc_dfm, "electricity")
temperature_usage <- track_term_over_time(rsc_dfm, "temperature") 
evolution_usage <- track_term_over_time(rsc_dfm, "evolution")

# Display emergence patterns
cat("Electricity usage over time:\n")
print(electricity_usage)

cat("\nTemperature usage over time:\n")
print(temperature_usage)

cat("\nEvolution usage over time:\n")
print(evolution_usage)

# Visualise concept emergence
library(ggplot2)

plot_concept_emergence <- function(usage_data, term_name) {
  ggplot(usage_data, aes(x = time_period)) +
    geom_line(aes(y = usage_rate), color = "blue", size = 1) +
    geom_point(aes(y = usage_rate), color = "blue", size = 2) +
    geom_line(aes(y = document_penetration * 100), color = "red", size = 1, linetype = "dashed") +
    geom_point(aes(y = document_penetration * 100), color = "red", size = 2) +
    scale_y_continuous(
      name = "Usage Rate (per 1000 words)",
      sec.axis = sec_axis(~./100, name = "Document Penetration (%)")
    ) +
    labs(
      title = paste("Emergence and Spread of '", term_name, "'", sep = ""),
      subtitle = "Blue = usage intensity, Red = breadth of adoption",
      x = "Decade"
    ) +
    theme_minimal() +
    theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
plot_concept_emergence(electricity_usage, "electricity")
plot_concept_emergence(temperature_usage, "temperature")
plot_concept_emergence(evolution_usage, "evolution")
...
- Usage Rate: How intensively is a term used when it appears (occurrences per 1000 words)
- Document Penetration: How widely distributed is the term across different papers

Things one can look for:

- Sudden emergence: Terms that appear rarely then spike dramatically
- Gradual adoption: Concepts that slowly gain acceptance over decades
- Early vs. late adoption: Whether terms start in specialist contexts then spread
- Peak and decline: Whether certain concepts become less central over time

### 5.2 Authorial Expertise: Who Were the Key Figures for Specific Topics?

Instead of comparing predefined author groups, we can identify which authors were most central to specific scientific concepts or fields.

```{r text_as_var2}
# Find key authors for different scientific concepts
electricity_authors <- find_key_authors_for_term(rsc_dfm, "electricity", min_usage = 3)
temperature_authors <- find_key_authors_for_term(rsc_dfm, "temperature", min_usage = 5)
magnetic_authors <- find_key_authors_for_term(rsc_dfm, "magnetic", min_usage = 3)

cat("Top authors discussing 'electricity':\n")
print(head(electricity_authors, 10))

cat("\nTop authors discussing 'temperature':\n")
print(head(temperature_authors, 10))

cat("\nTop authors discussing 'magnetic' phenomena:\n")
print(head(magnetic_authors, 10))

# Cross-reference with known historical figures
# (This would be where domain knowledge becomes valuable)
cat("\nUsage intensity shows how central a concept was to an author's work\n")
cat("(total mentions / total words written * 1000)\n")
```

### 5.3 Lexical Innovation: Tracking the Introduction of Technical Vocabulary

We can identify when new technical terms first appeared and trace their adoption patterns.

```{r text_var3}
# Find terminology that emerged in different centuries
innovations_1800s <- find_period_innovations(
  rsc_dfm, 
  early_period = c(1600, 1650, 1700), 
  late_period = c(1800, 1850), 
  time_var = "century"
)

innovations_1900s <- find_period_innovations(
  rsc_dfm,
  early_period = c(1600, 1700, 1800),
  late_period = c(1900),
  time_var = "century"
)

cat("Top 20 lexical innovations appearing in 1800s:\n")
print(head(innovations_1800s, 20))

cat("\nTop 20 lexical innovations appearing in 1900s:\n")
print(head(innovations_1900s, 20))

# Focus on high-frequency innovations (likely important concepts)
cat("\nHigh-frequency innovations in 1800s (used > 20 times):\n")
innovations_1800s %>% 
  filter(docs_using  > 20) %>%
  head(20)

cat("\nHigh-frequency innovations in 1900s (used > 20 times):\n")
innovations_1900s %>% 
  filter(docs_using  > 20) %>%
  head(20)
...
### 5.4 Diffusion Patterns: How Ideas Spread Through Scientific Networks

We can use text patterns to understand how concepts moved between different scientific communities or topics.

```{r text_var4}
# Track how key concepts spread across topics
electricity_by_topic <- track_concept_diffusion(rsc_dfm, "electricity", "primaryTopic")
temperature_by_topic <- track_concept_diffusion(rsc_dfm, "temperature", "primaryTopic")

cat("'Electricity' usage across primary topics:\n")
print(head(electricity_by_topic, 10))

cat("\n'Temperature' usage across primary topics:\n")
print(head(temperature_by_topic, 10))

# Visualise cross-topic diffusion
plot_topic_diffusion <- function(diffusion_data, term_name, top_n = 8) {
  diffusion_data %>%
    filter(total_usage > 0) %>%
    slice_head(n = top_n) %>%
    ggplot(aes(x = reorder(group, usage_rate), y = usage_rate)) +
    geom_col(fill = "steelblue", alpha = 0.7) +
    coord_flip() +
    labs(
      title = paste("Usage of '", term_name, "' Across Topics", sep = ""),
      subtitle = "Rate per 1000 words in each topic area",
      x = "Primary Topic",
      y = "Usage Rate (per 1000 words)"
    ) +
    theme_minimal()
}

plot_topic_diffusion(electricity_by_topic, "electricity")
plot_topic_diffusion(temperature_by_topic, "temperature")
```

### Text as Evidence for Broader Phenomena

This approach demonstrates how textual patterns can serve as evidence for historical, social, and intellectual processes. Rather than treating text as the object of study, we use it as a lens for understanding the metadata itself.

We can use this to generate empirically-grounded hypotheses about historical processes that can then be investigated using traditional historical methods—creating a productive dialogue between computational and humanistic approaches to understanding the past.

## 6. Network/Community Metadata: Intellectual Communities

**Research Questions:**

- How do different scholarly communities use language differently?
- What concepts are community-specific vs. shared across scientific networks?
- How do intellectual networks influence the spread of terminology?
- Which communities serve as bridges between different scientific domains?

**Methodological Strenght(s):** Network metadata reveals the social structure underlying scientific knowledge production. Different communities of scientists may develop distinct vocabularies, research methods, or ways of describing phenomena—even when studying related topics.

```{r network-analysis}
# NB: Community zero represents isolates - needs to be dropped!
# Get the largest two communities for comparison
...
cat("Analysing communities:", paste(large_communities, collapse = ", "), "\n\n")

# Subset DFM by YEARS and AUTHORS 

# Create community-based subsets
dfm_comm1 <- subset_corpus(rsc_dfm, communities = large_communities[1])
dfm_comm2 <- subset_corpus(rsc_dfm, communities = large_communities[2])

# Overview of community subsets
cat("Community", large_communities[1], "Corpus:\n")
cat("Documents:", ndoc(dfm_comm1), "\n")
cat("Tokens:", sum(ntoken(dfm_comm1)), "\n")

cat("\nCommunity", large_communities[2], "Corpus:\n")
cat("Documents:", ndoc(dfm_comm2), "\n") 
cat("Tokens:", sum(ntoken(dfm_comm2)), "\n\n")
...
And, again, we can turn to keyness to explore differences - but this time between communities active over the same time period.

```{r keyness_community}
# Find community-distinctive terms using keyness
combined_dfm <- rbind(dfm_comm1, dfm_comm2)
docvars(combined_dfm, "subset") <- c(rep("subset1", ndoc(dfm_comm1)),
                                        rep("subset2", ndoc(dfm_comm2)))
...
### Interpreting Community Results

Keyness reveals:

- Community-specific vocabularies: Terms that strongly characterise each intellectual network
- Disciplinary boundaries: How different scientific domains maintain distinct languages
- Methodological differences: Communities may prefer different research approaches or instruments
- Theoretical orientations: Distinct ways of conceptualising similar phenomena

These differences reflect how scientific communities develop their own research traditions, methodological preferences, and ways of understanding the natural world.

### Community Makeup

We are also able to look at who the people are that make up these communities, what they wrote, and what they wrote about.

```{r community_details}
# NB: Community zero represents isolates - needs to be dropped!
# Get the largest communities for meaningful comparison
slice_communities <- network_data %>%
  filter(slice_1725_1774 != 0) %>%
  filter(node_type == "author", !is.na(slice_1725_1774)) %>%
  count(slice_1725_1774, sort = TRUE) %>%
  #head(5) %>%
  pull(slice_1725_1774)

# Create community-based subsets (chose 6 and 8 for illustrative purposes)
dfm_comm1 <- subset_corpus(rsc_dfm, communities = slice_communities[6], time_slice = "slice_1725_1774")
dfm_comm2 <- subset_corpus(rsc_dfm, communities = slice_communities[8], time_slice = "slice_1725_1774")

# Overview of community subsets
cat("Community", slice_communities[6], "Corpus:\n")
cat("Documents:", ndoc(dfm_comm1), "\n")
cat("Tokens:", sum(ntoken(dfm_comm1)), "\n")

cat("\nCommunity", slice_communities[8], "Corpus:\n")
cat("Documents:", ndoc(dfm_comm2), "\n") 
cat("Tokens:", sum(ntoken(dfm_comm2)), "\n\n")

# Compare vocabularies
cat("Top 15 Terms in Community", slice_communities[6], ":\n")
topfeatures(dfm_comm1, 25) %>% print()

cat("\nTop 15 Terms in Community", slice_communities[8], ":\n")
topfeatures(dfm_comm2, 25) %>% print()

# The titles of five random documents
cat("The titles of five random documents\n")
docvars(dfm_comm1)$title[c(sample(c(1:ndoc(dfm_comm1)), 5))]
docvars(dfm_comm2)$title[c(sample(c(1:ndoc(dfm_comm1)), 5))]

# The (computationally assigned) topics for documents in the communities
cat("The (computationally assigned) topics for documents in the communities\n")
sort(table(docvars(dfm_comm1)$primaryTopic), decreasing = TRUE)
sort(table(docvars(dfm_comm2)$primaryTopic), decreasing = TRUE)

# The top 5 authors in each community
cat("The top 5 authors in each community\n")
sort(table(unlist(docvars(dfm_comm1)$authors_list)), decreasing = TRUE)[1:5]
sort(table(unlist(docvars(dfm_comm2)$authors_list)), decreasing = TRUE)[1:5]

```

And let's plot these networks:

```{r}
edges_comm1 <- make_bipartite_edges(dfm_comm1)
edges_comm2 <- make_bipartite_edges(dfm_comm2)

plot_bipartite(edges_comm1, slice_communities[6])
plot_bipartite(edges_comm2, slice_communities[8])

plot_coauthors(edges_comm1, slice_communities[6])
plot_coauthors(edges_comm2, slice_communities[8])

plot_coauthors_with_topics(dfm_comm1, edges_comm1, slice_communities[6])
plot_coauthors_with_topics(dfm_comm2, edges_comm2, slice_communities[8])
```

# Summary

What we've tried to demonstrate here is that **metadata can fundamentally determines what research questions are possible to ask and answer when using text.** Each type of metadata creates new analytical possibilities (or lenses). 

## Implications for Text Analysis

- **1. Research Design:** Always begin by cataloging available metadata and considering what questions each dimension enables
- **2. Data Collection:** Invest heavily in metadata acquisition as it's potentially more valuable than additional raw text
- **3. Analytical Strategy:** Design analysis plans that start with basic questions and allow for the addition of dimensions progressively
- **4. Interpretation:** Context from metadata is essential for moving from pattern detection to meaningful explanation
- **5. Reproducibility:** Document metadata schemas and subsetting criteria as carefully as analytical procedures

## Practical Implications and strategies for your research

### Two general approaches

**Exploratory Data Analysis (EDA)**

- Start simple: basic frequency analysis
- Build Complexity by layering different metadata dimensions
- Ask bigger questions (maybe… research questions) as you develop an understanding of your data and the substantive context

**Start with your research question(s)**

- Guided by domain expertise
- What do you want to know?
- What metadata would enable that analysis?
- Work backwards from question to data requirements

### Concluding notes

**Metadata is not optional**

- Determines analytical possibilities
- Invest time in understanding (and enriching) metadata

**Layer your analysis**

- Begin with simple, single-dimensional analyses
- Gradually combine metadata types for richer insights
- Each combination unlocks new research possibilities

**Document structure can be metadata**

Even without explicit categories, text structure provides context, e.g.:

- Network analysis from co-occurrence patterns
- Temporal patterns from publication sequences
- Topic modeling to categorise document 
- Text itself (tokens) can be used to understand metadata
...
# Step 2: Create your subsets:
my_dfm1 <- subset_corpus(rsc_dfm, decade = my_decade1, topics = my_topic)
my_dfm2 <- subset_corpus(rsc_dfm, decade = my_decade2, topics = my_topic)

# Step 3: Analyse and compare (top features, keyness)
...
<!-- ## Authorship Metadata: Individual vs Collaborative Work -->

<!-- **Research Questions Enabled**:  -->

<!-- - How does collaboration affect scientific language and argumentation? -->
<!-- - Do single authors write differently than research teams? -->
<!-- - What linguistic patterns emerge in collaborative vs. individual work? -->
<!-- - How does authorship structure relate to the complexity or certainty of scientific claims? -->

<!-- Historical Context: The transition from single-author to multi-author publications represents a major shift in how scientific knowledge is produced. In the Royal Society period, most work was still individual, making this an ideal dataset for studying this transition. -->

<!-- ```{r authorship-analysis} -->

<!-- # Create authorship-based subsets -->
<!-- dfm_single <- subset_corpus(rsc_dfm, single_author = TRUE) -->
<!-- dfm_multi <- subset_corpus(rsc_dfm, multi_author = TRUE) -->

<!-- # Overview of authorship subsets -->
<!-- cat("Single-Author Corpus:\n") -->
<!-- cat("Documents:", ndoc(dfm_single), "\n") -->
<!-- cat("Tokens:", sum(ntoken(dfm_single)), "\n") -->

<!-- cat("\nMulti-Author Corpus:\n") -->
<!-- cat("Documents:", ndoc(dfm_multi), "\n") -->
<!-- cat("Tokens:", sum(ntoken(dfm_multi)), "\n\n") -->
...
<!-- ### Interpreting Authorship Results: -->

<!-- Consider: -->

<!-- - Personal vs. collective voice: How does individual vs. collaborative authorship affect scientific rhetoric? -->
<!-- - Certainty and hedging: Do collaborative works show more cautious language? -->
<!-- - Methodological complexity: Are multi-author works more methodologically sophisticated? -->
<!-- - Institutional language: How do collaboration patterns affect references to institutions, equipment, or resources? -->

<!-- In all of these cases, *this might be an interesting area where stopwords are not dropped.* The patterns that emerge may reveal how the social organisation of scientific work influences the language used to communicate findings and arguments. -->
